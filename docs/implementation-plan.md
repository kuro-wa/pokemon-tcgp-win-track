...
- **依存タスク**: T-004, T-003
- **実施内容**:
  - テスト作成: 勝ち時の基本加算（+10）、連勝ボーナス（連勝1: +13, 連勝2: +16, 連勝3: +19, 連勝4+: +22）、負け時の減算（-5、暫定仕様）、引き分け時の不変（0）、ポイント下限0の境界値、ポイント上限なしの確認
  - テスト失敗確認
  - 最小実装: 勝利時 `基本増加値(10) + min(記録前連勝数, 4) × 3`、敗北時 `-5`（暫定仕様）、引き分け時 `0`
